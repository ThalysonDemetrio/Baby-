...
(function initPixModal() {
  ...
  function openModal(titulo) {
    if (!modal) return;
    if (metaNameEl) {
      metaNameEl.textContent = titulo.startsWith('Você está') ? titulo : 'Para: ' + titulo;
    }
  ...
  // Botões PIX em todos os cards (delegação de eventos para suportar cards criados dinamicamente)
  document.addEventListener('click', e => {
    const btnPix = e.target.closest('.btn-pix');
    if (btnPix) {
      openModal(btnPix.dataset.titulo || 'Contribuição');
      return;
    }
    const btnDonativo = e.target.closest('.btn-donativo');
    if (btnDonativo) {
      openModal('Você está contribuindo para: ' + (btnDonativo.dataset.titulo || 'Este Item'));
      return;
    }
  });
...
/* ════════════════════════════════════════
   8. CARRINHO DE BEBÊ (lista de presentes)
════════════════════════════════════════ */
(function initCarrinhoRegistry() {
  async function fetchItensEnxoval() {
    const grid = document.getElementById('carrinhoGrid');
    if (!grid) return;

    // Dados de fallback caso o fetch falhe (produção/local estático)
    const mockItens = [
      { nome_item: 'Carrinho de Bebê', meta_valor: 1200.00, valor_arrecadado: 600.00, icone_path: '🛒', concluido: false },
      { nome_item: 'Banheira com Suporte', meta_valor: 250.00, valor_arrecadado: 250.00, icone_path: '🛁', concluido: true },
      { nome_item: 'Cadeirão de Alimentação', meta_valor: 400.00, valor_arrecadado: 150.00, icone_path: '🪑', concluido: false },
      { nome_item: 'Kit de Mamadeiras', meta_valor: 180.00, valor_arrecadado: 180.00, icone_path: '🍼', concluido: true },
      { nome_item: 'Bolsa Maternidade', meta_valor: 300.00, valor_arrecadado: 90.00, icone_path: '👜', concluido: false }
    ];

    let itemsData = [];
    try {
      // Tenta realizar requisição assíncrona ao endpoint da API
      const response = await fetch('/api/itens-enxoval');
      if (response.ok) {
        itemsData = await response.json();
      } else {
        throw new Error('Endpoint indisponível. Utilizando banco mock local.');
      }
    } catch (error) {
      console.warn('[Carrinho de Bebê] Fetch falhou, carregando mock local:', error.message);
      itemsData = mockItens;
    }

    grid.innerHTML = '';
    itemsData.forEach(item => {
      const meta = Number(item.meta_valor);
      const arrecadado = Number(item.valor_arrecadado);
      const percent = Math.min((arrecadado / meta) * 100, 100);
      const isCompleted = item.concluido || percent >= 100;

      const card = document.createElement('div');
      card.className = `carrinho-card${isCompleted ? ' completed' : ''}`;

      card.innerHTML = `
        <div class="carrinho-card-header">
          <div class="carrinho-icon-wrap" aria-hidden="true">${item.icone_path || '🎁'}</div>
          ${isCompleted ? '<span class="carrinho-badge-completed">Concluído! 🎁</span>' : ''}
        </div>
        <h4 class="carrinho-card-title">${item.nome_item}</h4>
        <div class="carrinho-progress-wrap">
          <div class="carrinho-progress-bar" role="progressbar" 
               aria-valuenow="${arrecadado}" aria-valuemin="0" aria-valuemax="${meta}" 
               aria-label="Progresso: R$${arrecadado} de R$${meta}">
            <div class="carrinho-progress-fill" style="width: 0%" data-width="${percent}"></div>
          </div>
          <div class="carrinho-values-wrap">
            <span class="carrinho-values">
              <span class="carrinho-raised">R$ ${arrecadado.toLocaleString('pt-BR')}</span>
              de R$ ${meta.toLocaleString('pt-BR')}
            </span>
            <span class="carrinho-percent">${percent.toFixed(0)}%</span>
          </div>
        </div>
        <button type="button" class="btn-donativo" data-titulo="${item.nome_item}" ${isCompleted ? 'disabled' : ''}>
          ${isCompleted ? 'Item Garantido! 🎉' : 'Doar este Item via PIX'}
        </button>
      `;
    ...
    // Animação lazy das barras de progresso ao renderizar
    setTimeout(() => {
      grid.querySelectorAll('.carrinho-progress-fill').forEach(fill => {
        fill.style.width = fill.getAttribute('data-width') + '%';
      });
    }, 100);
  }

  // Observa a seção de produtos para carregar apenas quando visível
  const target = document.getElementById('carrinhoGrid');
  if (target) {
